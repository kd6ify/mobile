...
import com.futureconcepts.drake.client.constants.ContactListManagerConstants;
...
{
	private static final String LOG_TAG = ContactListView.class.getSimpleName();
    Activity mScreen;
    IImConnection mConn;
    SimpleAlertHandler mHandler;
    Context mContext;
    private final IContactListListener mContactListListener;
    private final IChatGroupListener mChatGroupListener;
    ...
    public ContactListView(Context screen, AttributeSet attrs) {
        ...
        mContactListListener = new MyContactListListener();
        mChatGroupListener = new MyChatGroupListener();
    }

    private class MyContactListListener extends IContactListListener.Stub
    {
        @Override
    ...
    {
        @Override
        public void onChatGroupCreated(String address)
        {
        }
        ...
        if (mConn != conn)
        {
            ...
            if (conn != null)
            {
                ...
                try
                {
                    ...
                    if (listMgr.getState() == ContactListManagerConstants.LISTS_LOADED)
                    {
                        mAdapter.startAutoRequery();
                    }
                }
    ...
    public void startChat(Cursor c)
    {
        if (c != null)
        {
            ...
            int contactType = c.getInt(c.getColumnIndexOrThrow(Imps.Contacts.TYPE));
            Log.d("ContactListView.startChat", "contactType=" + contactType);
            try {
                IChatSessionManager manager = mConn.getChatSessionManager();
                IChatSession session = manager.getChatSession(username);
                if(session == null)
                {
//                	if (contactType == Imps.Contacts.TYPE_GROUP)
//                	{
//                		session = manager.createGroupChatSession(username);
//                	}
//                	else
                	{
    ...
    public boolean isContactsLoaded() {
        try {
            ...
            return (manager.getState() == ContactListManagerConstants.LISTS_LOADED);
        } catch (RemoteException e) {
            mHandler.showServiceErrorAlert();
            return false;
        }
    ...
    public void removeContact(Cursor c) {
        if (c == null) {
    ...
    public void blockContact(Cursor c) {
        if (c == null) {
            mHandler.showAlert(R.string.error, R.string.select_contact);
        } else {
            String nickname = c.getString(c.getColumnIndexOrThrow(Imps.Contacts.NICKNAME));
    ...
    private void registerListeners()
    {
        try
        {
            ...
            IChatGroupManager chatGroupManager = mConn.getChatGroupManager();
            chatGroupManager.registerChatGroupListener(mChatGroupListener);
        }catch(RemoteException e) {
            mHandler.showServiceErrorAlert();
        }
    }
    ...
    private final OnChildClickListener mOnChildClickListener = new OnChildClickListener() {
        public boolean onChildClick(ExpandableListView parent, View v, int groupPosition,
            ...
            if (cursor == null) {
                Log.w(LOG_TAG,
                        "[OnChildClickListener.onChildClick] cursor null! groupPos="
            ...
            {
            	int type = cursor.getInt(ContactView.COLUMN_CONTACT_TYPE);
                if (type == Imps.Contacts.TYPE_GROUP)
                {
                	Intent joinIntent = new Intent(mScreen, JoinChatGroupActivity.class);
                	joinIntent.setData(ContentUris.withAppendedId(Imps.Contacts.CONTENT_URI, cursor.getLong(ContactView.COLUMN_CONTACT_ID)));
                	mScreen.startActivity(joinIntent);
                }
                else
                {
                	startChat(cursor);
    ...
    public void setAutoRefreshContacts(boolean isRefresh)
    {
