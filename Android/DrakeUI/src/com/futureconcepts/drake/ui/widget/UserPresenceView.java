...
public class UserPresenceView extends LinearLayout {
    ...
    private StatusIconAdapter getStatusAdapter() {
        try {
            ...
            for (int i = 0; i < supportedStatus.length; i++) {
                ...
                Drawable icon = mContext.getResources().getDrawable(PresenceUtils.getStatusIconId(s));
                String text = mContext.getResources().getString(PresenceUtils.getStatusStringRes(s));
    ...
    void updateStatusText() {
        ...
        mStatusBar = initStatusBar(false);
    }
    ...
    private void updateView() {
        ...
        mStatusDialogButton.setImageDrawable(mContext.getResources().getDrawable(PresenceUtils.getStatusIconId(status)));

        String statusText = mPresence.getStatusText();
        if (TextUtils.isEmpty(statusText)) {
            statusText = mContext.getString(PresenceUtils.getStatusStringRes(status));
        }
    ...
    private TextView initStatusBar(boolean showEdit)
    {
        ...
        if (showEdit)
        {
           ...
           statusEdit.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_NORMAL);
