...
public abstract class ContactListManager
{
     ...
     */
    public ContactList getContactListByAddress(String address)
    {
    	for (ContactList list : mContactLists)
    	{
     ...
     * @throws ImException
     */
    public synchronized void createContactListAsync(String name, Collection<Contact> contacts, boolean isDefault) throws ImException
    {
        checkState();

        if (getContactListByName(name) != null)
        {
    ...
    {
        deleteContactListAsync(getContactListByName(name));
    }
    ...
    {
        blockContactAsync(contact.getAddress());
    }
    ...
    {
        unblockContactAsync(contact.getAddress());
    }
    ...
    {
        return isBlocked(contact.getAddress());
    }
    ...
    protected void notifyContactError(int type, ImErrorInfo error,
        ...
        if (type == ContactListListener.ERROR_REMOVING_CONTACT) {
            ...
            mBlockPending.remove(contact.getAddress());
        }
        for (ContactListListener listener : mContactListListeners) {
