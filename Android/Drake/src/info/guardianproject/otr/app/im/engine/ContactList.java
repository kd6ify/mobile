...
public class ContactList extends ImEntity
{
    ...
    public ContactList(String address, String name, boolean isDefault, Collection<Contact> contacts, ContactListManager manager)
    {
        mAddress = address;
        mDefault = isDefault;
        mName = name;
        ...
        if (contacts != null)
        {
            ...
            {
                mContactsCache.put(manager.normalizeAddress(c.getAddress()), c);
            }
        }
    }
     ...
     */
    public void removeContact(String address) throws ImException
    {
        if(address == null)
        {
            throw new NullPointerException();
    ...
    {
        return address == null ? false : mContactsCache.containsKey(mManager.normalizeAddress(address));
    }

    public synchronized boolean containsContact(Contact c)
    {
        return c == null ? false
                : mContactsCache.containsKey(mManager.normalizeAddress(c.getAddress()));
    }

    protected void insertToCache(Contact contact)
    {
        mContactsCache.put(mManager.normalizeAddress(contact.getAddress()), contact);
    }

    protected void removeFromCache(Contact contact)
    {
        mContactsCache.remove(mManager.normalizeAddress(contact.getAddress()));
    }
}
