...
public abstract class FileTransfer {
	...
	protected void setStatus(Status status) {
        synchronized (statusMonitor) {
		    ...
		    {
		    	statusChangedListener.onStatusChanged(this);
		    }
	    }
    }
			...
			throws XMPPException
    {
		...
		Log.d("FileTransfer.writeToStream", "HERE");
        do {
			...
			try {
				count = in.read(b);
				Log.d("FileTransfer.writeToStream", "in.read got " + count);
			} catch (IOException e) {
				throw new XMPPException("error reading from input stream", e);
			}
		} while (count != -1 && !getStatus().equals(Status.cancelled));

        Log.d("FileTransfer.writeToString", "out of loop");
        Log.d("FileTransfer.writeToString", "status=" + getStatus().toString());
        Log.d("FileTransfer.writeToString", "error=" + getError().toString());
        Log.d("FileTransfer.writeToString", "amountWritten=" + amountWritten);
        Log.d("FileTransfer.writeToString", "fileSize=" + fileSize);
    ...
    {
    	void onStatusChanged(FileTransfer fileTransfer);
    }
}
