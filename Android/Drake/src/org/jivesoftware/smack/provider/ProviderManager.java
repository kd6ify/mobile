...
public class ProviderManager {
    ...
    protected void initialize() {
        ...
        try {
        	Context context = AndroidContext.getInstance(null).getContext();
        	XmlPullParser parser = context.getResources().getXml(com.futureconcepts.drake.R.xml.smack_providers);
            parser.setFeature(XmlPullParser.FEATURE_PROCESS_NAMESPACES, true);
            int eventType = parser.getEventType();
            do {
                if (eventType == XmlPullParser.START_TAG) {
                    if (parser.getName().equals("iqProvider")) {
                    	parseIqProvider(parser);
                    }
                    else if (parser.getName().equals("extensionProvider")) {
                    	parseExtensionProvider(parser);
                    }
                }
    ...
    private void parseExtensionProvider(XmlPullParser parser) throws XmlPullParserException, IOException, IllegalAccessException, InstantiationException
    {
        int eventType = 0;
    ...
    private void parseIqProvider(XmlPullParser parser) throws XmlPullParserException, IOException, IllegalAccessException, InstantiationException
    {
        int eventType = 0;
        String elementName = null;
        String namespace = null;
