...
public class DownloadService extends Service {
    ...
    private class UpdateThread extends Thread {
        ...
        public void run() {
            ...
            for (;;) {
                    ...
                    if (!mPendingUpdate)
                    {
                        ...
                        if (wakeUp != Long.MAX_VALUE)
                        {
                            ...
                            else
                            {
                                ...
                                alarms.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + wakeUp, PendingIntent.getBroadcast(DownloadService.this, 0, intent,
                ...
                Cursor cursor = getContentResolver().query(Downloads.Impl.CONTENT_URI, null, null, null, Downloads.Impl._ID);
                ...
                while (!isAfterLast || arrayPos < mDownloads.size()) {
                    if (isAfterLast) {
                        ...
                        if (arrayPos == mDownloads.size()) {
                            ...
                            if (Constants.LOGVV) {
                                Log.v(Constants.TAG, "Array update: appending " + id + " @ " + arrayPos);
                            }
    ...
    private void trimDatabase() {
        Cursor cursor = getContentResolver().query(Downloads.Impl.CONTENT_URI,
                ...
                Downloads.Impl.STATUS + " >= '200'", null,
    ...
    {
        int statusColumn = cursor.getColumnIndexOrThrow(Downloads.Impl.STATUS);
        ...
        DownloadInfo info = new DownloadInfo(
                ...
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl.URI)),
                cursor.getInt(cursor.getColumnIndexOrThrow(
                        Downloads.Impl.NO_INTEGRITY)) == 1,
                cursor.getString(cursor.getColumnIndexOrThrow(
                        Downloads.Impl.FILENAME_HINT)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl._DATA)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl.MIME_TYPE)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.DESTINATION)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.VISIBILITY)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.CONTROL)),
                ...
                cursor.getString(cursor.getColumnIndexOrThrow(
                        Downloads.Impl.NOTIFICATION_EXTRAS)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl.COOKIE_DATA)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl.USER_AGENT)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.Impl.REFERER)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.TOTAL_BYTES)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.CURRENT_BYTES)),
        ...
        if (info.canUseNetwork(networkAvailable, networkRoaming)) {
            if (info.isReadyToStart(now)) {
                ...
                if (info.mStatus != Downloads.Impl.STATUS_RUNNING) {
                    ...
                    values.put(Downloads.Impl.STATUS, info.mStatus);
            ...
            if (info.mStatus == 0
                ...
                ContentValues values = new ContentValues();
                values.put(Downloads.Impl.STATUS, Downloads.Impl.STATUS_RUNNING_PAUSED);
    ...
    private void updateDownload(
        ...
        info.mUri = stringFromCursor(info.mUri, cursor, Downloads.Impl.URI);
        info.mNoIntegrity = cursor.getInt(cursor.getColumnIndexOrThrow(
                Downloads.Impl.NO_INTEGRITY)) == 1;
        info.mHint = stringFromCursor(info.mHint, cursor, Downloads.Impl.FILENAME_HINT);
        info.mFileName = stringFromCursor(info.mFileName, cursor, Downloads.Impl._DATA);
        info.mMimeType = stringFromCursor(info.mMimeType, cursor, Downloads.Impl.MIME_TYPE);
        ...
        info.mPackage = stringFromCursor(
                info.mPackage, cursor, Downloads.Impl.NOTIFICATION_PACKAGE);
        info.mClass = stringFromCursor(
                info.mClass, cursor, Downloads.Impl.NOTIFICATION_CLASS);
        info.mCookies = stringFromCursor(info.mCookies, cursor, Downloads.Impl.COOKIE_DATA);
        info.mUserAgent = stringFromCursor(
                info.mUserAgent, cursor, Downloads.Impl.USER_AGENT);
        info.mReferer = stringFromCursor(info.mReferer, cursor, Downloads.Impl.REFERER);
