...
public class DirectoryScanner extends Thread
{
	...
	public DirectoryScanner(File directory, Context context, Handler handler, MimeTypes mimeTypes, String sdCardPath)
	{
	...
	public void run()
	{
			...
			for (File currentFile : files){ 
        	  ...
        	  if (currentFile.isHidden()) {
				...
				if (currentFile.isDirectory()) { 
					...
					String mimetype = mMimeTypes.getMimeTypeExplicit(fileName);
					if (mimetype != null)
					{
		...
		if (!cancel) {
			...
			Message msg = handler.obtainMessage(SaveFileActivity.MESSAGE_SHOW_DIRECTORY_CONTENTS);
			msg.obj = contents;
			msg.sendToTarget();
		}
	...
	private void updateProgress(int progress, int maxProgress) {
		...
		if ((progress % PROGRESS_STEPS) == 0) {
			...
			// Okay, send an update.
			Message msg = handler.obtainMessage(SaveFileActivity.MESSAGE_SET_PROGRESS);
			msg.arg1 = progress;
