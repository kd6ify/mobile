...
import org.jivesoftware.smack.util.PacketParserUtils2;
...
public class SmackParser {
    ...
    public Packet parse(Stanza stanza) throws Exception {
        if ("presence".equals(stanza.getName())) {
            return PacketParserUtils2.parsePresence(getParser(stanza));
        }
        if ("iq".equals(stanza.getName())) {
            return PacketParserUtils2.parseIQ(
                    getParser(stanza),
                    DUMMY_CONNECTION
            );
        }
        if ("message".equals(stanza.getName())) {
            return PacketParserUtils2.parseMessage(getParser(stanza));
        }
        return null;
    }
