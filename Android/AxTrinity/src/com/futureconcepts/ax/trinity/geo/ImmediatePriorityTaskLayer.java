...
public class ImmediatePriorityTaskLayer extends Layer
{
	private static final String TAG = ImmediatePriorityTaskLayer.class.getSimpleName();
	private Tactic _tactic;
	private int _size;
	private int[] _positionMap;
	private int _itemCount = -1;
	
	public ImmediatePriorityTaskLayer(Context context, MapView mapView, Tactic tactic)
	{
		super(mapView);
		_tactic = tactic;
		_size = size();
	...
	public boolean onTap(int i)
	{
		...
		getMapView().getContext().startActivity(new Intent(Intent.ACTION_VIEW, uri));
		return true;
	}
	...
	private int size()
	{
		if (_itemCount == -1)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				Address address = _tactic.getAddress(getMapView().getContext());
				if (address != null)
				{
	...
	private View createMarker(final int i)
	{
		...
		MapView.LayoutParams params = new MapView.LayoutParams(MapView.LayoutParams.WRAP_CONTENT, MapView.LayoutParams.WRAP_CONTENT,
				getWKTAsGeoPoint(_tactic), MapView.LayoutParams.BOTTOM_CENTER);
		imageView.setLayoutParams(params);
		
		TacticPriority tacticPriority = _tactic.getPriority(getMapView().getContext());
	...
	private GeoPoint getWKTAsGeoPoint(Tactic tactic)
	{
		GeoPoint result = null;
		Address address = tactic.getAddress(getMapView().getContext());
		if (address != null)
		{
