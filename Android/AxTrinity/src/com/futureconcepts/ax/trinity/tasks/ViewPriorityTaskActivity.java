...
public class ViewPriorityTaskActivity extends ViewItemActivity
{
    ...
    private ViewItemDescriptor[] _myItems = {
		...
		new ViewItemDescriptor( "Priority", ITEM_TYPE_INDEXED_TYPE, Tactic.PRIORITY),
		new ViewItemDescriptor( "Status", ITEM_TYPE_INDEXED_TYPE, Tactic.STATUS),
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        if (getData() != null)
        {
	    	...
	    	{
	    		setTitle("Priority Task: " + _tactic.getName());
	...
	{
		final String[] options = {"Assets","Collectives","Show On Map","Edit Priority Task"};
		...
		builder.setItems(options,new DialogInterface.OnClickListener() {					
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if("Edit Priority Task".equals(options[which]))
						{
							Intent edit = new Intent(ViewPriorityTaskActivity.this ,AddEditPriorityTaskActivity.class);
						...
						}else if("Show On Map".equals(options[which]))
						{
							if (_tactic != null)
					    	{
								if(_tactic.isMappable(ViewPriorityTaskActivity.this))
								{
									onMenuShowOnMap();
								}else{
								Toast.makeText(getApplicationContext(), "No address found for this Priority Task.",
    ...
    private final class MyAdapter extends ViewItemAdapter
    {
    	public MyAdapter()
    	{
    		...
    		DateTime.setContext(ViewPriorityTaskActivity.this);
		...
		public View getView(int position, View convertView, ViewGroup parent)
		{
			...
			if (vid.type == MY_ADDRESS_TYPE)
			{
				result = getAddressView(vid, _tactic.getAddress(ViewPriorityTaskActivity.this));
			}
			else if (vid.type == ITEM_TYPE_INDEXED_TYPE)
			{
				if (vid.displayName.equals("Type"))
				{
					result = getIndexedTypeView(vid, _tactic.getType(ViewPriorityTaskActivity.this));
				}
				else if (vid.displayName.equals("Priority"))
				{
					result = getIndexedTypeView(vid, _tactic.getPriority(ViewPriorityTaskActivity.this));
				}
				else
				{
					result = getIndexedTypeView(vid, _tactic.getStatus(ViewPriorityTaskActivity.this));
				}
			}
			else
			{
